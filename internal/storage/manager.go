...
func (sm *storageManager) getAliOSSCredentials(ctx context.Context, namespace string, secretRef v1.SecretRef) (string, string, error) {
	...
	if secretRef.Key != "" {
		accessKeyKey = secretRef.Key + "-accessKeyId"
		secretKeyKey = secretRef.Key + "-accessKeySecret"
	}
	...
	if secretKey == "" {
		secretKey = string(secret.Data["accessKeySecret"])
	}

	if accessKey == "" || secretKey == "" {
		return "", "", fmt.Errorf("在Secret %s 中缺少accessKeyId或accessKeySecret", secretRef.Name)
	}
...
func (sm *storageManager) getTencentCOSCredentials(ctx context.Context, namespace string, secretRef v1.SecretRef) (string, string, error) {
	...
	if secretRef.Key != "" {
		secretIDKey = secretRef.Key + "-secretId"
	...
	if secretID == "" || secretKey == "" {
		return "", "", fmt.Errorf("在Secret %s 中缺少secretId或secretKey", secretRef.Name)
	}
